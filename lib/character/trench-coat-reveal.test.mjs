...
function recordingCanvas() {
  ...
  const canvas = {
    ...
    restore() { this.globalAlpha = alphaStack.pop() ?? 1; },
    beginPath() {}, closePath() {}, clip() {}, fill() {}, stroke() {},
    moveTo(x, y) { if (this.globalAlpha > 0) points.push({ x, y }); },
    lineTo(x, y) { if (this.globalAlpha > 0) points.push({ x, y }); },
    quadraticCurveTo(cx, cy, x, y) { if (this.globalAlpha > 0) points.push({ x: cx, y: cy }, { x, y }); },
    arc(x, y, radius) { if (this.globalAlpha > 0) points.push({ x: x - radius, y }, { x: x + radius, y }); },
    fillRect(x, y, width, height) { if (this.globalAlpha > 0) points.push({ x, y }, { x: x + width, y: y + height }); },
    strokeRect(x, y, width, height) { if (this.globalAlpha > 0) points.push({ x, y }, { x: x + width, y: y + height }); },
    fillText() {},
  };
  return canvas;
}
...
test("the reveal image fills most of the opened coat", () => {
  const torso = 53;
  const bounds = trenchCoatRevealBounds(torso, -torso);
  assert.ok(bounds.width > torso * 1.5, "reveal should be substantially wider than the torso");
  assert.ok(bounds.height > torso * 1.6, "reveal should extend from chest toward the coat hem");
  assert.ok(bounds.y + bounds.height < torso * 0.8, "reveal should remain inside the lower hem");
});
