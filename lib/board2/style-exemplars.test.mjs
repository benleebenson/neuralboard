...
test("extracts collage geometry, title, marginalia, and broad camera movement", () => {
  const manifest = exampleManifest();
  manifest.board.dimensions = { width: 4000, height: 3000 };
  manifest.board.media.forEach((media, index) => Object.assign(media, {
    boardX: [200, 700, 2600, 3200][index],
    boardY: [250, 600, 1800, 2200][index],
    boardW: index === 0 ? 900 : 300,
    boardH: index === 0 ? 600 : 200,
  }));
  manifest.annotations.push({ type: "circle" }, { type: "text", text: "DREAM PRACTICE", boardX: 1300, boardY: 1200, boardW: 1400, boardH: 300, fontSize: 140 });
  manifest.camera.keyframes = [
    { time: 0, broadPan: true }, { time: 3, broadPan: true },
    { time: 12, broadPan: true }, { time: 15, broadPan: true },
  ];
  const summary = extractBoardStyleSummary(manifest);
  assert.equal(summary.layout.distinctMediaCount, 4);
  assert.equal(summary.layout.largestToMedianArea, 9);
  assert.ok(summary.layout.whitespaceRatio > 0.9);
  assert.equal(summary.layout.titleCenterpiece.present, true);
  assert.equal(summary.annotations.byType.circle, 1);
  assert.equal(summary.camera.broadPanCount, 2);
});
