...
test("maps the narration media clock onto the exact board timeline", () => {
  const clip = { id: "voice", startTime: 12, duration: 20, sourceOffsetSec: 5 };
  assert.equal(narrationTimelineTimeFromMedia(clip, 5), 12);
  assert.equal(narrationTimelineTimeFromMedia(clip, 8.25), 15.25);
  assert.equal(narrationTimelineTimeFromMedia(clip, 100), 32);
  assert.equal(narrationTimelineTimeFromMedia(clip, 0), 12);
});
