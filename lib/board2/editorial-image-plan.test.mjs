...
test("extracts a transcript-specific board title from planner output", () => {
  assert.equal(parseEditorialBoardTitle('{"title":"Lucid Dreaming: No Baggage","topics":[]}'), "Lucid Dreaming: No Baggage");
  assert.equal(parseEditorialBoardTitle('[{"topicTitle":"Dreams"}]'), null);
});
...
test("the prompt includes the full narration, timestamps, and editorial constraints", () => {
  ...
  assert.match(prompt.system, /boardTitle/);
  assert.match(prompt.system, /SCOPE axis/);
  assert.match(prompt.system, /RELATIVE TO THIS VIDEO'S SUBJECT/);
  assert.match(prompt.system, /NO required number or cadence of BROAD beats/);
  assert.match(prompt.system, /"narrationText"/);
...
test("parses a subject title and retains its repeated image beats", () => {
  const response = JSON.stringify({ boardTitle: "The Apollo Program", topics: [{
    topicTitle: "First Moon Landing", startTime: 0, endTime: 12,
    images: [
      { query: "Apollo lunar module photograph", startTime: 0, reason: "Arrival." },
      { query: "Apollo lunar module photograph", startTime: 6, reason: "Returns to the craft." },
    ],
  }] });
  assert.equal(parseEditorialBoardTitle(response), "The Apollo Program");
  assert.equal(parseEditorialTopicPlan(response, 12).flatMap((topic) => topic.images).length, 2);
});

test("parses semantic scope case-insensitively and maps old shot plans safely", () => {
  const plan = parseEditorialImagePlan(JSON.stringify([
    { query: "psychologist laboratory portrait photograph", startTime: 0, reason: "Specific person.", narrationText: "Dr. Hall ran this experiment.", scope: "NARROW" },
    { query: "psychology lecture hall wide photograph", startTime: 4, reason: "Field-level transition.", narrationText: "But what did this mean for psychology?", scope: "BROAD" },
    { query: "notebook detail photograph", startTime: 8, reason: "Legacy detail.", shot: "tight" },
  ]), 12, 3);
  assert.deepEqual(plan.map((beat) => beat.scope), ["narrow", "broad", "narrow"]);
  assert.equal(plan[1].narrationText, "But what did this mean for psychology?");
});
...
test("rejects malformed output while retaining repeated appearances", () => {
  ...
  assert.equal(plan[0].startTime, 0);
  assert.equal(plan[1].startTime, 3);
  assert.equal(plan[2].startTime, 9.9);
});
