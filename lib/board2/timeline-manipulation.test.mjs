...
for (const type of visualTypes) {
  test(`loaded ${type} ignores board-only media on every timeline layer`, () => {
    const active = { id: "active", type, startTime: 5, duration: 1.5, layer: 1 };
    const boardOnlyMedia = Array.from({ length: 5 }, (_, layer) => ({
      id: `board-only-${layer}`,
      type: "image",
      startTime: 0,
      duration: 4,
    ...
    for (let targetLayer = 0; targetLayer < 5; targetLayer += 1) {
      const result = applyTimelineBlockDrag([active, ...boardOnlyMedia], {
        kind: "move",
        blockId: active.id,
        originalStartTime: active.startTime,
        originalDuration: active.duration,
        originalLayer: active.layer,
      }, {
        deltaSeconds: 2,
        targetLayer,
        playhead: 5,
      ...
      assert.equal(result[0].startTime, 7);
      assert.equal(result[0].layer, targetLayer);
    }

    const moveTo = (deltaSeconds) => applyTimelineBlockDrag([active, ...boardOnlyMedia], {
      kind: "move",
      blockId: active.id,
      originalStartTime: active.startTime,
      originalDuration: active.duration,
    ...
    assert.equal(moveTo(-2), 3);
    assert.equal(moveTo(-20), 0);
  });
}
