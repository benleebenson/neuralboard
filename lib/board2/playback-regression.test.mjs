...
test("continuous narration follows its media clock and every resume uses an exact seek boundary", () => {
  ...
  const playbackLoop = bodyBetween("const rafLoop = useCallback", "useEffect(() => { rafCallbackRef.current");
  assert.match(playbackLoop, /narrationTimelineTimeFromMedia\(narrationClip, narrationElement\.currentTime\)/);
  assert.doesNotMatch(source, /useEffect\(\(\) => \{ playheadRef\.current = playhead; \}, \[playhead\]\)/);
});

test("space is reserved for board panning and never toggles playback", () => {
  const spaceHandler = bodyBetween('if (e.code === "Space") {', "if (inInput) return;");
  assert.match(spaceHandler, /isSpaceDownRef\.current = true/);
  assert.doesNotMatch(spaceHandler, /togglePlay/);
});
...
test("real-time compatibility export predecodes images and culls off-camera work", () => {
  ...
  assert.match(source, /const DETERMINISTIC_EXPORT_ENABLED = process\.env\.NEXT_PUBLIC_DETERMINISTIC_EXPORT !== "0"/);
});

test("deterministic WebCodecs export is frame-indexed and independent of wall-clock RAF", () => {
  ...
  assert.match(deterministic, /exportFrameTime\(frameIndex, fps\)/);
  assert.match(deterministic, /editorCameraAtTime\(frameTime/);
  assert.match(deterministic, /renderToCtx\(exportCtx, frameTime/);
  assert.match(deterministic, /new VideoFrame/);
  assert.doesNotMatch(deterministic, /requestAnimationFrame/);
