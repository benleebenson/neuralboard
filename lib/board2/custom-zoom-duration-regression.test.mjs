...
test("new custom zoom blocks use the current preference without changing other defaults", () => {
  ...
  assert.match(customZoomCreation, /resolveCustomZoomInsertion/);
  ...
  assert.match(panCreation, /resolveManualTimelineInsertion\(prev, requestedStart, duration\)/);
  ...
  assert.match(characterFocusCreation, /resolveManualTimelineInsertion\(prev, requestedStart, duration\)/);
});
