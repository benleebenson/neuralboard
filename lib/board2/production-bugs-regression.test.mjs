...
test("auto-build image requests stay sequential, retry once, and expose slow progress", () => {
  const imageSearch = bodyBetween("const found: Array<", "reportProgress(\"placing\", \"Placing images…\"");
  assert.match(imageSearch, /for \(let index = 0; index < segments\.length; index\+\+\)/);
  assert.match(imageSearch, /await requestAutoBuildImage/);
  assert.doesNotMatch(imageSearch, /Promise\.all/);
  assert.match(imageSearch, /this can take several minutes/);
  assert.match(imageSearch, /describeImageSuccess/);
  assert.match(imageSearch, /describeImageSkip/);
});

test("auto-build exposes persistent weighted progress and recoverable per-slot state", () => {
  const panel = bodyBetween("function AutoBuildProgressPanel", "type SpeechBubbleCue");
  assert.match(panel, /data-auto-build-progress/);
  assert.match(panel, /role="progressbar"/);
  assert.match(panel, /formatAutoBuildDuration/);
  assert.match(panel, /autoBuildRemainingMs/);
  assert.match(panel, /data-auto-build-slot-state/);
  assert.match(panel, /Cancel auto-build/);

  assert.match(source, /AUTO_BUILD_PROGRESS_STORAGE_KEY/);
  assert.match(source, /sessionStorage\.setItem\(AUTO_BUILD_PROGRESS_STORAGE_KEY/);
  assert.match(source, /status: "failure"[\s\S]*Auto-build was interrupted by a page reload/);
  assert.match(source, /Bridge unreachable — is the Mac mini awake and the tunnel running\?/);
});

test("auto-build checks bridge health before spending transcription/planning cost, and surfaces its status and a failed-slot retry path", () => {
  const build = bodyBetween("async function autoBuildFromNarration", "async function retryFailedAutoBuildSlots");
  assert.match(build, /checkBridgeHealth\(controller\.signal\)/);
  assert.ok(build.indexOf("checkBridgeHealth") < build.indexOf("await compileNarrationToBlob"));

  const retry = bodyBetween("async function retryFailedAutoBuildSlots", "async function generateNarrationLipSync");
  assert.match(retry, /requestAutoBuildImage/);
  assert.match(retry, /buildNarrationLockedImageWindows/);
  assert.match(retry, /setKeyframesOutOfDate\(true\)/);

  const panel = bodyBetween("function AutoBuildProgressPanel", "type SpeechBubbleCue");
  assert.match(panel, /onRetryFailed/);
  assert.match(panel, /Retry \{failedSlots\}/);

  assert.match(source, /Bridge: \{bridgeStatus/);
  assert.match(source, /checkBridgeHealth\(\)\.catch/);
});
...
test("trench-coat reveals start in place and expose image and timing controls", () => {
  const addReveal = bodyBetween("function addTrenchCoatRevealSequence", "function addExplodeSequence");
  assert.match(addReveal, /const targetX = currentPose\.boardX/);
  assert.match(addReveal, /const targetY = currentPose\.boardY/);
  assert.match(addReveal, /sequenceSetupDuration: 0/);
  assert.doesNotMatch(addReveal, /editorCameraAtTime|resolveGroundY|WALK_SPEED/);
  assert.match(source, /REVEAL — click to upload an image/);
  assert.match(source, /aria-label="Time before trench-coat reveal"/);
  assert.match(source, /handleTrenchCoatRevealDividerPointerDown/);
});

test("the trench coat suppresses the permanent wrist launcher", () => {
  const renderer = readFileSync(new URL("../character/board-renderer.ts", import.meta.url), "utf8");
  const launcher = renderer.slice(renderer.indexOf("// Permanent wrist launcher:"), renderer.indexOf("// Neck/head chain."));
  assert.match(launcher, /p\.sequenceRenderer !== "trenchCoatReveal"/);
});
