...
import { DEBUG_STREAM, LIVE_FEATURES_ENABLED, STREAM_OWNER_NAME, STREAM_OWNER_USER_ID } from "@/app/board2/config";
...
const BAZOOKA_PROJECTILE_SPEED = 1100;
const NAMED_REJOIN_STORAGE_KEY = `neuralboard:stream:${STREAM_OWNER_USER_ID}:named-rejoin-required`;
const ANONYMOUS_GUEST_NAME = "Guest";
...
function resolveGuestSpawn(snapshot: StreamSnapshotMessage) {
  ...
  if (snapshot.spawnDoor) {
    const { x, y } = snapshot.spawnDoor;
  ...
  const doorX = snapshot.board.width / 2;
  const doorY = snapshot.board.height / 2;
...
export default function StreamPage() {
  ...
  const receiveBazookaFire=useCallback((event:StreamBazookaFireMessage)=>{
    ...
    if(characterImpact)return;
    pendingBazookaVisualShotsRef.current.set(key,{
      event,
      previousDistance:0,
      maxDistance:Math.hypot(event.target.x-event.from.x,event.target.y-event.from.y),
      terrainDistance:terrainImpact?Math.hypot(terrainImpact.point.x-event.from.x,terrainImpact.point.y-event.from.y):Infinity,
    });
  },[]);
  const receiveBazookaImpact=useCallback((event:StreamBazookaImpactMessage)=>{
    ...
    pendingBazookaVisualShotsRef.current.delete(event.shotId);
    bazookaCharacterImpactsRef.current.set(event.shotId,event);
    window.setTimeout(()=>bazookaCharacterImpactsRef.current.delete(event.shotId),15000);
    ...
    if(event.target.role==="guest"&&event.target.guestId!==guestId){
      const frame=remoteGuests.current.get(event.target.guestId);
      if(frame)guestBazookaVisualImpactsRef.current.set(event.target.guestId,{event,startPosition:{...frame.position}});
    }
  ...
  const updateBazookaVisualCollisions=(nowMs:number)=>{
    if(pendingBazookaVisualShotsRef.current.size===0)return;
    for(const [key,shot] of pendingBazookaVisualShotsRef.current){
      if(bazookaCharacterImpactsRef.current.has(key)){
        pendingBazookaVisualShotsRef.current.delete(key);
        continue;
      }
      const traveled=clamp((nowMs-shot.event.startTime)/1000*BAZOOKA_PROJECTILE_SPEED,0,shot.maxDistance);
      if(traveled<=shot.previousDistance)continue;
      const dx=shot.event.target.x-shot.event.from.x,dy=shot.event.target.y-shot.event.from.y,length=Math.max(1,Math.hypot(dx,dy)),direction={x:dx/length,y:dy/length};
      const segmentFrom={x:shot.event.from.x+direction.x*shot.previousDistance,y:shot.event.from.y+direction.y*shot.previousDistance};
      const segmentTo={x:shot.event.from.x+direction.x*traveled,y:shot.event.from.y+direction.y*traveled};
      const targets=[];
      const hostFrame=latestHost.current?.characters.find(character=>character.id==="c1"&&character.enabled);
      if(hostFrame&&shot.event.shooter?.role!=="host")targets.push(...bazookaCharacterHitZones({role:"host",x:hostFrame.x,y:hostFrame.y}));
      for(const [id,frame] of remoteGuests.current){
        if(id===shot.event.shooter?.guestId||eliminations.current.has(id))continue;
        targets.push(...bazookaCharacterHitZones({role:"guest",guestId:id,x:frame.position.x,y:frame.position.y}));
      }
      const local=physics.current;
      if(local&&mode==="guest"&&guestId!==shot.event.shooter?.guestId&&!eliminations.current.has(guestId)){
        targets.push(...bazookaCharacterHitZones({role:"guest",guestId,x:local.x,y:local.y}));
      }
      const hit=firstBazookaCharacterHit(segmentFrom,segmentTo,targets);
      const hitDistance=hit?shot.previousDistance+hit.distance:Infinity;
      if(hit&&hitDistance<shot.terrainDistance){
        const existing=bazookaEventsRef.current.find(candidate=>bazookaShotKey(candidate)===key);
        if(existing){
          bazookaEventsRef.current=[
            ...bazookaEventsRef.current.filter(candidate=>bazookaShotKey(candidate)!==key),
            {...existing,target:hit.point,fizzle:false},
          ].slice(-12);
        }
        pendingBazookaVisualShotsRef.current.delete(key);
        continue;
      }
      if(shot.terrainDistance<=traveled||traveled>=shot.maxDistance){
        pendingBazookaVisualShotsRef.current.delete(key);
        continue;
      }
      pendingBazookaVisualShotsRef.current.set(key,{...shot,previousDistance:traveled});
    }
  };
  ...
  useEffect(()=>{
    const initialize=window.setTimeout(()=>{
      ...
      try{requiresName=localStorage.getItem(NAMED_REJOIN_STORAGE_KEY)==="1";}catch{}
      setNamedRejoinRequired(requiresName);
      if(requiresName)setMode("join");
    },0);
    return()=>window.clearTimeout(initialize);
  },[]);
  const requireNamedRejoin=useCallback((message:string)=>{
    try{localStorage.setItem(NAMED_REJOIN_STORAGE_KEY,"1");}catch{}
    autoJoinAttempted.current=true;
    setNamedRejoinRequired(true);
    setName("");
    setMode("join");
  ...
  useEffect(()=>{const initial=window.setTimeout(()=>void loadSnapshot(),0);const supabase=getBrowserSupabase();const channelName=streamChannelName(STREAM_OWNER_USER_ID);if(!supabase){streamDebugLog("realtime not configured",{channel:channelName});window.setTimeout(()=>setStatus("realtime-not-configured"),0);return()=>window.clearTimeout(initial);}const key=`client-${guestId}`;streamDebugLog("join channel",{channel:channelName,key});const channel=supabase.channel(channelName,{config:{broadcast:{self:false},presence:{key}}});channelRef.current=channel;
    ...
    channel.on("broadcast",{event:"snapshot"},({payload})=>{streamDebugLog("snapshot broadcast",{sessionId:(payload as StreamSnapshotMessage).sessionId,clips:(payload as StreamSnapshotMessage).clips?.length});snapshotRef.current=payload as StreamSnapshotMessage;setSnapshot(snapshotRef.current);setLive(true);setStatus("live");})
      ...
      .on("broadcast",{event:"repair_board"},()=>{cratersRef.current=[];streamDebugLog("repair board guest");})
  ...
  const beginGuest=async(requestedName=name,anonymous=false):Promise<boolean>=>{const safe=cleanName(requestedName);if(!safe){setJoinError("Enter a short, appropriate name.");return false;}const guests=participants.filter(p=>p.role==="guest");if(guests.length>=MAX_GUESTS){setJoinError("This room is full.");return false;}const currentSnapshot=snapshotRef.current??snapshot;if(!currentSnapshot){setJoinError("Waiting for the live board…");return false;}nameRef.current=safe;setName(safe);if(face&&!faceCache.current.has(guestId)){const img=new Image();img.src=face;faceCache.current.set(guestId,img);}if(signDataUrlRef.current)cacheSignImage(guestId,signDataUrlRef.current);const spawn=resolveGuestSpawn(currentSnapshot);physics.current={x:spawn.x,y:spawn.y,vx:0,vy:0,targetX:null,targetY:null,facing:1,grounded:spawn.grounded,surfaceId:spawn.surfaceId,action:"idle",actionStarted:performance.now(),spawnAt:performance.now(),physique:"slim"};camera.current={cameraX:spawn.x,cameraY:spawn.y-120,boardZoom:1.35};guestSeq.current=0;const joinedAt=Date.now();const presence={role:"guest",isHost:false,guestId,name:safe,faceDataUrl:face,skin:hostGuestSkin.current,physique:"slim",signDataUrl:validSignDataUrl(signDataUrlRef.current)?signDataUrlRef.current:undefined,joinedAt} satisfies StreamParticipantPresence;streamDebugLog("presence track send",presence);await channelRef.current?.track(presence);autoJoinAttempted.current=true;if(!anonymous){try{localStorage.removeItem(NAMED_REJOIN_STORAGE_KEY);}catch{}setNamedRejoinRequired(false);}setMode("guest");setJoinError("");return true;};
  useEffect(()=>{
    if(namedRejoinRequired!==false||mode!=="landing"||!live||!snapshot||subscribeStatus!=="SUBSCRIBED"||autoJoinAttempted.current)return;
    autoJoinAttempted.current=true;
    void beginGuest(ANONYMOUS_GUEST_NAME,true);
    // The readiness values below are the only events that should trigger the one-time automatic spawn.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[namedRejoinRequired,mode,live,snapshot,subscribeStatus]);
      ...
      try {
        ...
        if (p && snapshot) {
          ...
          if (!planned && held?.phase !== "hold") {
            ...
            if(p.grounded&&p.action!=="ragdoll"&&!horizontalMotion.hitX)p.y=horizontalMotion.y;
        ...
        updateBazookaVisualCollisions(Date.now());
        ...
        if (snapshot) {
          for (const clip of [...snapshot.clips].sort((a, b) => (a.layer ?? 1) - (b.layer ?? 1))) {
            ...
            if (img?.complete&&img.naturalWidth>0) drawCrateredImage(ctx,img,x,y,sw,sh,clip.boardW,clip.boardH,cratersRef.current.filter(crater=>crater.clipId===clip.id));
          ...
          for (const [id, g] of remoteGuests.current) {
            ...
            const bazookaVisual=guestBazookaVisualImpactsRef.current.get(id);
            if(bazookaVisual&&(g.actionType==="ragdoll"||Date.now()-bazookaVisual.event.impactAt>500))guestBazookaVisualImpactsRef.current.delete(id);
            const activeBazookaVisual=guestBazookaVisualImpactsRef.current.get(id);
            const bazookaAge=activeBazookaVisual?Math.max(0,(Date.now()-activeBazookaVisual.event.impactAt)/1000):0;
            const bazookaFrame=activeBazookaVisual?{
              ...
              actionParams:{ragdollSpin:activeBazookaVisual.event.spin,ragdollPhase:"airborne",terrainGrounded:false},
              weapon:g.weapon?{...g.weapon,armed:false}:undefined,
            }:null;
            const renderFrame = choke?.phase === "hold"
              ...
              : choke?.phase === "drop"
                ...
                : bazookaFrame ?? (event ? eliminationFrameForGuest(g, event, Date.now(), hostClockOffset.current) : g);
            if (!renderFrame) {
          ...
          if (p && mode === "guest") {
            ...
            if (renderFrame) {
              ...
              requireNamedRejoin(`💥 KICKED by ${event?.hostName || "Host"} — enter a name to rejoin.`);
            }
          }
  ...
  }, [guestId, mode, hostCam, snapshot, requireNamedRejoin]);
  ...
  if(!LIVE_FEATURES_ENABLED)return <main style={landing}><section style={card}><h1>{STREAM_OWNER_NAME}</h1><p>Live streaming and guest join are currently unavailable.</p></section></main>;
  if(mode==="join")return <main style={landing}><form style={card} onSubmit={e=>{e.preventDefault();void beginGuest(name,false);}}><h2>{namedRejoinRequired?"Enter a name to rejoin":"Join as character"}</h2><input autoFocus required maxLength={GUEST_NAME_MAX_LENGTH} value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" style={input}/><label style={{...button,display:"block",textAlign:"center",marginTop:10}}>Optional face<input hidden type="file" accept="image/*" onChange={async e=>{const f=e.target.files?.[0];if(f)setFace(await bakeFace(f));}}/></label>{face&&<img src={face} alt="Face preview" style={{width:48,height:56,display:"block",margin:"10px auto"}}/>}<p style={{fontSize:10,color:"#8b2b20"}}>{joinError}</p><button type="submit" disabled={!live||!snapshot} style={{...button,background:live?"#c8f135":"#ddd"}}>Spawn</button><button type="button" style={button} onClick={()=>setMode("landing")}>Cancel</button></form></main>;
  if(mode==="landing")return <main style={landing}><section style={card}><div style={{fontSize:11,color:live?"#228b22":"#8a6a00",fontWeight:700}}>{live?"LIVE":status==="ended"?"STREAM ENDED":status==="reconnecting"?"RECONNECTING":"OFFLINE"}</div><h1>{STREAM_OWNER_NAME}</h1><p>{live?(joinError||"Joining the live board…"):status==="reconnecting"?"reconnecting…":joinError||"No active stream right now."}</p><button disabled={!live||!snapshot} style={{...button,background:live?"#c8f135":"#ddd"}} onClick={()=>{if(namedRejoinRequired)setMode("join");else void beginGuest(ANONYMOUS_GUEST_NAME,true);}}>{namedRejoinRequired?"Enter a name to rejoin":`Join now (${participants.filter(p=>p.role==="guest").length}/${MAX_GUESTS})`}</button></section></main>;
