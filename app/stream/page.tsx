...
export default function StreamPage() {
  ...
  const latestHost=useRef<StreamFrameMessage|null>(null), remoteGuests=useRef(new Map<string,GuestCharacterFrame>()), renderedGuests=useRef(new Map<string,GuestCharacterFrame>()), physics=useRef<GuestPhysics|null>(null), camera=useRef<StreamCamera>({cameraX:2000,cameraY:1500,boardZoom:1}), publishAt=useRef(0), emoteIndex=useRef(0);
  ...
  const loadSnapshot=useCallback(async()=>{const res=await fetch(`/api/stream/snapshot?streamId=${encodeURIComponent(STREAM_OWNER_USER_ID)}`,{cache:"no-store"});const data=await res.json();setLive(!!data.live);setSnapshot(data.snapshot??null);setStatus(data.live?"live":"offline");},[]);
  ...
  useEffect(()=>{const initial=window.setTimeout(()=>void loadSnapshot(),0);const supabase=getBrowserSupabase();if(!supabase){window.setTimeout(()=>setStatus("realtime-not-configured"),0);return()=>window.clearTimeout(initial);}const key=`client-${guestId}`;const channel=supabase.channel(streamChannelName(STREAM_OWNER_USER_ID),{config:{broadcast:{self:false},presence:{key}}});channelRef.current=channel;
    channel.on("broadcast",{event:"snapshot"},({payload})=>{setSnapshot(payload as StreamSnapshotMessage);setLive(true);setStatus("live");})
      .on("broadcast",{event:"frame"},({payload})=>{latestHost.current=payload as StreamFrameMessage;setLive(true);})
      ...
      .on("broadcast",{event:"session-end"},()=>{setLive(false);setStatus("ended");physics.current=null;setMode("landing");})
      .on("presence",{event:"sync"},()=>{const rows=Object.values(channel.presenceState()).flat() as unknown as StreamParticipantPresence[];setParticipants(rows);for(const p of rows)if(p.guestId&&p.faceDataUrl&&!faceCache.current.has(p.guestId)){const img=new Image();img.src=p.faceDataUrl;faceCache.current.set(p.guestId,img);}})
      .subscribe(async s=>{if(s==="SUBSCRIBED"){await channel.track({role:"viewer",joinedAt:Date.now()} satisfies StreamParticipantPresence);void loadSnapshot();}else if(s==="CHANNEL_ERROR"||s==="TIMED_OUT")setStatus("reconnecting");});return()=>{window.clearTimeout(initial);supabase.removeChannel(channel);channelRef.current=null;};},[guestId,loadSnapshot]);
