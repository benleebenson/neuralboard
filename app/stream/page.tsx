...
export default function StreamPage() {
  ...
  const bazookaEventsRef=useRef<StreamBazookaFireMessage[]>([]),cratersRef=useRef<StreamCrater[]>([]);
  ...
  useEffect(()=>{const initial=window.setTimeout(()=>void loadSnapshot(),0);const supabase=getBrowserSupabase();const channelName=streamChannelName(STREAM_OWNER_USER_ID);if(!supabase){streamDebugLog("realtime not configured",{channel:channelName});window.setTimeout(()=>setStatus("realtime-not-configured"),0);return()=>window.clearTimeout(initial);}const key=`client-${guestId}`;streamDebugLog("join channel",{channel:channelName,key});const channel=supabase.channel(channelName,{config:{broadcast:{self:false},presence:{key}}});channelRef.current=channel;
    ...
    channel.on("broadcast",{event:"snapshot"},({payload})=>{streamDebugLog("snapshot broadcast",{sessionId:(payload as StreamSnapshotMessage).sessionId,clips:(payload as StreamSnapshotMessage).clips?.length});snapshotRef.current=payload as StreamSnapshotMessage;setSnapshot(snapshotRef.current);setLive(true);setStatus("live");})
      ...
      .on("broadcast",{event:"bazooka_fire"},({payload})=>{const event=payload as StreamBazookaFireMessage;bazookaEventsRef.current=[...bazookaEventsRef.current,event].slice(-12);const clip=snapshotRef.current?.clips.filter(c=>c.type==="image"&&event.target.x>=c.boardX&&event.target.x<=c.boardX+c.boardW&&event.target.y>=c.boardY&&event.target.y<=c.boardY+c.boardH).sort((a,b)=>(b.layer??1)-(a.layer??1))[0],impactDelay=Math.max(0,event.startTime+Math.hypot(event.target.x-event.from.x,event.target.y-event.from.y)/1100*1000-Date.now());window.setTimeout(()=>{if(event.startTime<repairAtRef.current)return;if(clip){const crater=craterForImpact(clip,event.target,event.seed);cratersRef.current=[...cratersRef.current.filter(c=>c.clipId!==clip.id),...cratersRef.current.filter(c=>c.clipId===clip.id).slice(-11),crater];streamDebugLog("bazooka crater guest",crater);}const p=physics.current;if(p&&Math.hypot(p.x-event.target.x,p.y-90-event.target.y)<140){p.vx+=(p.x<event.target.x?-1:1)*320;p.vy=-280;p.grounded=false;p.action="jump";}},impactDelay);streamDebugLog("bazooka fire guest",event);})
