...
function guestFrameFromPhysics(p: GuestPhysics, guestId: string, name: string, sessionId: string, now: number): GuestCharacterFrame {
...
export default function StreamPage() {
  ...
  const beginGuest=async()=>{const safe=cleanName(name);if(!safe){setJoinError("Enter a short, appropriate name.");return;}const guests=participants.filter(p=>p.role==="guest");if(guests.length>=MAX_GUESTS){setJoinError("This room is full.");return;}if(!snapshot)return;nameRef.current=safe;if(face&&!faceCache.current.has(guestId)){const img=new Image();img.src=face;faceCache.current.set(guestId,img);}const spawn=resolveGuestSpawn(snapshot);physics.current={x:spawn.x,y:spawn.y,vx:0,vy:0,targetX:null,targetY:null,facing:1,grounded:spawn.grounded,surfaceId:spawn.surfaceId,action:"idle",actionStarted:performance.now(),spawnAt:performance.now()};camera.current={cameraX:spawn.x,cameraY:spawn.y-120,boardZoom:1.35};await channelRef.current?.track({role:"guest",guestId,name:safe,faceDataUrl:face,skin:"styled",physique:"slim",joinedAt:Date.now()} satisfies StreamParticipantPresence);setMode("guest");setJoinError("");};
  ...
  const jumpGuest=()=>{const p=physics.current;if(!p||!p.grounded||!GUEST_VERB_SET.has("jump"))return;p.vy=-760;p.grounded=false;p.surfaceId=null;p.action="jump";p.actionStarted=performance.now();};
  const flipGuestTo=(targetX?:number,targetY?:number)=>{const p=physics.current;if(!p||!p.grounded||!GUEST_VERB_SET.has("flip"))return;const dx=targetX!==undefined?targetX-p.x:p.facing*520;p.facing=dx>=0?1:-1;p.vx=clamp(Math.abs(dx)*1.35,420,760)*p.facing;p.vy=-clamp(targetY!==undefined?680+Math.max(0,p.y-targetY)*.55:760,700,900);p.grounded=false;p.surfaceId=null;p.targetX=targetX??p.x+p.facing*520;p.targetY=targetY??p.y;p.action="flip";p.actionStarted=performance.now();};
    ...
    const frame = (now: number) => {
      try {
        ...
        if (p && now - publishAt.current > 1000 / STREAM_FPS && host) {
          ...
          const packet = guestFrameFromPhysics(p, guestId, nameRef.current, host.sessionId, now);
  ...
  const clickCanvas=(e:React.MouseEvent<HTMLCanvasElement>)=>{const p=physics.current;if(!p||!snapshot||hostCam||(p.frozenUntil&&Date.now()<p.frozenUntil))return;const rect=e.currentTarget.getBoundingClientRect(),cam=camera.current,sf=cam.boardZoom*rect.width/BOARD_W,rawX=(e.clientX-rect.left-rect.width/2)/sf+cam.cameraX,rawY=(e.clientY-rect.top-rect.height/2)/sf+cam.cameraY;const surfaces=streamSurfaces(snapshot).filter(s=>rawX>=s.boardX&&rawX<=s.boardX+s.boardW);const destination=surfaces.sort((a,b)=>Math.abs(a.boardY-rawY)-Math.abs(b.boardY-rawY))[0];const targetX=destination?clamp(rawX,destination.boardX+18,destination.boardX+destination.boardW-18):clamp(rawX,0,snapshot.board.width);const targetY=destination?.boardY??p.y;if(e.detail>=2||guestHeldKeys.current.has("f")){flipGuestTo(targetX,targetY);return;}const wantsJump=p.grounded&&rawY<p.y-90;const jumpsToHigherSurface=!!destination&&p.grounded&&p.y-destination.boardY>=60&&p.y-destination.boardY<=280;if((wantsJump||jumpsToHigherSurface)&&GUEST_VERB_SET.has("jump")){const height=jumpsToHigherSurface&&destination?Math.max(0,p.y-destination.boardY):120;p.vy=-clamp(680+height*.5,680,850);p.grounded=false;p.surfaceId=null;p.action="jump";p.actionStarted=performance.now();}p.targetX=targetX;p.targetY=targetY;};
  useEffect(()=>{const isTyping=(target:EventTarget|null)=>target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement||target instanceof HTMLSelectElement;const down=(e:KeyboardEvent)=>{if(mode!=="guest"||isTyping(e.target))return;const p=physics.current;if(p?.frozenUntil&&Date.now()<p.frozenUntil)return;const key=e.key.toLowerCase();guestHeldKeys.current.add(key);if(key==="e")emote();if(key==="v")setHostCam(v=>!v);if(key==="j")jumpGuest();if(key==="d"&&p&&GUEST_VERB_SET.has("dance")){p.action="dance";p.actionStarted=performance.now();p.vx=0;p.targetX=null;}if(key==="f")e.preventDefault();};const up=(e:KeyboardEvent)=>{guestHeldKeys.current.delete(e.key.toLowerCase());};addEventListener("keydown",down);addEventListener("keyup",up);return()=>{removeEventListener("keydown",down);removeEventListener("keyup",up);guestHeldKeys.current.clear();};},[mode]);
  ...
  return <main style={{position:"fixed",inset:0,overflow:"hidden",background:"#f5ecd8"}}><canvas ref={canvasRef} onClick={clickCanvas} style={{width:"100vw",height:"100vh",display:"block",cursor:mode==="guest"&&!hostCam?"crosshair":"default"}}/><div style={{position:"fixed",top:12,left:12,display:"flex",gap:8,fontFamily:"monospace",alignItems:"flex-start",flexWrap:"wrap",maxWidth:"calc(100vw - 24px)"}}><span style={pill}>{mode==="guest"?name:`LIVE · ${STREAM_OWNER_NAME}`}</span>{mode==="guest"&&<><span style={pill}>Click walk/run · J jump · F+click/double-click flip · D dance · E emote</span><button style={pill} onClick={()=>setHostCam(v=>!v)}>{hostCam?"Host cam":"Follow me"} · V</button><button style={pill} onClick={emote}>Emote · E</button><button style={pill} onClick={leave}>Leave</button></>}</div></main>;
}
