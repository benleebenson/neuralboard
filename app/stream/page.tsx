...
function guestFrameFromPhysics(p: GuestPhysics, guestId: string, name: string, sessionId: string, now: number, signDataUrl?: string, signActive = false): GuestCharacterFrame {
...
export default function StreamPage() {
  ...
  const beginGuest=async()=>{const safe=cleanName(name);if(!safe){setJoinError("Enter a short, appropriate name.");return;}const guests=participants.filter(p=>p.role==="guest");if(guests.length>=MAX_GUESTS){setJoinError("This room is full.");return;}if(!snapshot)return;nameRef.current=safe;if(face&&!faceCache.current.has(guestId)){const img=new Image();img.src=face;faceCache.current.set(guestId,img);}if(signDataUrlRef.current)cacheSignImage(guestId,signDataUrlRef.current);const spawn=resolveGuestSpawn(snapshot);physics.current={x:spawn.x,y:spawn.y,vx:0,vy:0,targetX:null,targetY:null,facing:1,grounded:spawn.grounded,surfaceId:spawn.surfaceId,action:"idle",actionStarted:performance.now(),spawnAt:performance.now()};camera.current={cameraX:spawn.x,cameraY:spawn.y-120,boardZoom:1.35};const joinedAt=Date.now();await channelRef.current?.track({role:"guest",guestId,name:safe,faceDataUrl:face,skin:GUEST_DEFAULT_SKIN,physique:"slim",signDataUrl:validSignDataUrl(signDataUrlRef.current)?signDataUrlRef.current:undefined,signActive:!!signDataUrlRef.current&&signActiveRef.current,joinedAt} satisfies StreamParticipantPresence);setMode("guest");setJoinError("");};
    ...
    const frame = (now: number) => {
      try {
        ...
        if (p && now - publishAt.current > 1000 / STREAM_FPS && host) {
          ...
          const packet = guestFrameFromPhysics(p, guestId, nameRef.current, host.sessionId, now, validSignDataUrl(signDataUrlRef.current) ? signDataUrlRef.current : undefined, signActiveRef.current);
        ...
        if (snapshot) {
          ...
          if (p && mode === "guest") {
            ...
            const localFrame = guestFrameFromPhysics(p, guestId, nameRef.current, sessionId, now, validSignDataUrl(signDataUrlRef.current) ? signDataUrlRef.current : undefined, signActiveRef.current);
